...
class FrontendContractTest(unittest.TestCase):
    ...
    def test_hong_kong_timezone_and_deep_links(self):
        ...
        self.assertIn("以下日期時間為政府天氣稿發出時間", app)
        self.assertIn("新有效時間：", app)
        self.assertIn('class="event-heading"', app)
        self.assertIn('class="event-issued"', app)
        self.assertIn('class="event-action"', app)
        self.assertIn('datetime="${e.event_at}"', app)
        self.assertIn("eventDateTimeFmt", app)
        self.assertNotIn("data-event-body-toggle", app)
        self.assertNotIn("展開完整內容", app)
    ...
    def test_refreshed_theme_is_loaded_on_every_page(self):
        ...
        self.assertIn('.event-issued{display:block;margin-bottom:7px;color:var(--cyan)', theme)
        self.assertIn('.event-action{display:flex', theme)
        self.assertNotIn('.event-body.is-collapsed', theme)

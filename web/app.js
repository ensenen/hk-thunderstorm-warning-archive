...
function eventHtml(e){const until=e.valid_until?`<span>新有效時間：${eventDateTimeFmt(e.valid_until,e.event_at)}</span>`:'';return `<article class="event"><div class="event-dot"></div><div class="event-content"><header class="event-heading"><time class="event-issued" datetime="${e.event_at}">${eventDateFmt(e.event_at)} ${timeFmt(e.event_at)}</time><div class="event-action"><h3>${eventNames[e.event_type]||e.event_type}</h3><div class="event-valid">${until}${e.is_correction?'<span>更正稿</span>':''}</div></div></header><p class="event-body">${escapeHtml(e.body_text)}</p>${e.parse_warnings?.length?`<div class="detail-note">解析備註：${escapeHtml(e.parse_warnings.map(parseWarningName).join('；'))}</div>`:''}<a class="source-link" href="${e.source_url}" target="_blank" rel="noopener">政府天氣稿原文 ↗</a></div></article>`}

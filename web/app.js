...
function eventHtml(e){const until=e.valid_until?`<span>新有效時間：${dateTimeFmt(e.valid_until)}</span>`:'';return `<article class="event"><div class="event-dot"></div><div class="event-content"><header class="event-heading"><h3>${eventNames[e.event_type]||e.event_type}</h3><div class="event-timing"><span>天氣稿發出：<time datetime="${e.event_at}">${eventDateFmt(e.event_at)} ${timeFmt(e.event_at)}</time></span>${until}${e.is_correction?'<span>更正稿</span>':''}</div></header><p>${escapeHtml(e.body_text)}</p>${e.parse_warnings?.length?`<div class="detail-note">解析備註：${escapeHtml(e.parse_warnings.map(parseWarningName).join('；'))}</div>`:''}<a class="source-link" href="${e.source_url}" target="_blank" rel="noopener">政府天氣稿原文 ↗</a></div></article>`}
